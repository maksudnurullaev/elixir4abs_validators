defmodule Elixir4ABS.DecisionTableTest do
  ...
  describe "LoanOverdue.decide/1" do
    test "П5 Реструктуризация: списать долг, без штрафов (любые дни)" do
      assert LoanOverdue.decide(%{
               ...
               %{
                 debit_principal: true,
                 charge_penalty: false,
                 set_overdue_status: false,
                 send_sms: false,
                 transfer_to_legal: false
               }
    end
    ...
    test "П4 Дефолт (> 30 дней): пеня + статус + суд" do
      assert LoanOverdue.decide(%{
               ...
               %{
                 ...
                 set_overdue_status: true,
                 send_sms: false,
                 transfer_to_legal: true
               }
    end

    test "граница: 3 дня → Грейс, 4 дня → Просрочка" do
      grace =
        LoanOverdue.decide(%{days_overdue: 3, restructuring: false, balance_sufficient: false})

      overdue =
        LoanOverdue.decide(%{days_overdue: 4, restructuring: false, balance_sufficient: false})
  ...
  describe "DocumentAccess.decide/1" do
    ...
    test "П2 Конфиденциально: кредитный офицер — только просмотр + лог" do
      assert DocumentAccess.decide(%{
               doc_type: "credit_file",
               user_role: "credit_officer",
               retention_expired: false,
               client_status: "active"
             }) ==
               %{
  ...
  describe "ClientOnboarding.decide/1" do
    test "П1 Успех: все проверки пройдены → Active" do
      assert ClientOnboarding.decide(%{
               pinfl_valid: true,
               passport_valid: true,
               face_match: true,
  ...
  describe "ClientDeduplication.decide/1" do
    test "П1 Дубль: полное совпадение — запрет создания" do
      assert ClientDeduplication.decide(%{
               pinfl_match: "full",
               passport_match: "full",
               name_match: "full"
             }) ==
  ...
  describe "ClientLifecycle.decide/1" do
    test "П1 Просрочка паспорта → Inactive" do
      assert ClientLifecycle.decide(%{
               passport_expired: true,
               aml_listed: false,
               death_cert: false,
               accounts_closed_5y: false
             }) ==
               %{
                 set_inactive: true,
                 set_blocked: false,
                 set_closed_deceased: false,
                 set_archived: false
               }
    end
  ...
  describe "ProtocolTransformation.decide/1" do
    test "П1 Internal: модуль ABS → схема JSON (A)" do
      assert ProtocolTransformation.decide(%{
               source: "abs_module",
               target_format: "json",
               encryption: "tls"
             }) ==
