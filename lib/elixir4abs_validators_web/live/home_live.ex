  ...
  def mount(_params, _session, socket) do
    ...
    features = [
      %{
        ...
        description: "Проверка 20-значного расчётного счёта банков Узбекистана. " <>
        ...
        badge:       "2 метода",
        items:       ["Метод 1 — 7·1·3, mod 10", "Метод 2 — mod 11 (клиринг ЦБ)",
                      "Расшифровка структуры счёта", "Идентификатор НИББД"]
      },
      %{
        title:       "SWIFT ISO 20022 — pacs.008",
        description: "Парсинг и валидация FI-to-FI Customer Credit Transfer. " <>
        ...
        badge:       "ISO 20022",
        items:       ["Парсинг XML через SweetXml", "Проверка namespace pacs.008.001.08",
                      "Валидация IBAN, BIC, суммы, даты", "Поддержка множества транзакций"]
      },
      %{
        title:       "Генератор QR-платежей",
        description: "Имитация генерации платёжных QR-кодов для продавцов банка. " <>
        ...
        badge:       "QR / P2M",
        items:       ["Уникальный TXN ID · timestamp + 4 байта", "Схема APEX://pay?merchant=…",
                      "SVG QR — обновление в реальном времени", "5 примеров продавцов · TTL · валюта"]
      },
      %{
        title:       "Таблицы решений",
        description: "Бизнес-правила банка, скомпилированные из Elixir-макросов в байткод BEAM. " <>
  ...
  def render(assigns) do
    ...
    <div class="min-h-screen bg-gray-50">
      ...
      <div class="text-center text-xs text-gray-400 pb-8">
        ...
        <a href="https://github.com/maksudnurullaev/elixir4abs_validators"
           class="hover:text-gray-600 underline" target="_blank">GitHub</a>
      </div>
