defmodule Elixir4absValidatorsWeb.ContractPaymentLive do
  ...
  def handle_async(:api_call, {:exit, reason}, socket) do
    {:noreply,
     assign(socket, loading: false, error: "Процесс завершился с ошибкой: #{inspect(reason)}")}
  end
  ...
  defp build_request_from_assigns(a) do
    case a.tab do
      :get_contract ->
        ContractPayment.build_get_contract(
          a.gc_pinfl,
          parse_int(a.gc_type, 1),
          a.api_key,
          a.secret_key
        )
      ...
      :action ->
        ContractPayment.build_action(
          a.ac_txn_id,
          parse_int(a.ac_action, 2),
          a.api_key,
          a.secret_key
        )
  ...
  defp tab_desc(:get_contract),
    do: "GET /PaymentGateway/GetContract — данные контракта студента по ПИНФЛ"

  defp tab_desc(:create_payment),
    do: "POST /PaymentGateway/HandlePayment (action=1) — создание платежа"

  defp tab_desc(:action),
    do: "POST /PaymentGateway/HandlePayment (action=2/-2) — подтверждение или отмена"

  defp tab_desc(:get_status),
    do: "GET /PaymentGateway/GetPaymentStatus — статус платежа по ID транзакции"
              ...
              <%= if @tab == :get_contract do %>
                <form phx-change="rebuild" class="space-y-4">
                  ...
                  <div>
                    ...
                    >
                      <option value="1" selected={@gc_type == "1"}>
                        1 — CONTRACT (по умолчанию)
                      </option>
                      <option value="2" selected={@gc_type == "2"}>
                        2 — CREDIT (образовательный кредит)
                      </option>
                    </select>
                  </div>
                </form>
                ...
                <form phx-change="rebuild" class="space-y-4">
                  ...
                  <%= if @ac_action == "-2" do %>
                    <div class="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 text-xs text-amber-700">
                      Отмена возможна только для платежей в статусе <strong>ACCEPTED</strong>
                      (status_id=2).
              ...
              <%= if @tab == :get_status do %>
                ...
                <div class="mt-4 bg-gray-50 rounded-lg p-3 text-xs text-gray-500">
                  ...
                  <div class="space-y-1">
                    ...
                    <div>
                      <code class="bg-blue-100 text-blue-700 px-1 rounded">2</code>
                      — ACCEPTED · Принят, ожидает подтверждения
                    </div>
                    <div>
                      <code class="bg-green-100 text-green-700 px-1 rounded">3</code>
                      — TRANSFERRED · Средства зачислены
                    </div>
                    <div>
                      <code class="bg-amber-100 text-amber-700 px-1 rounded">4</code>
                      — RETURNED · Отменён
                    </div>
                    <div>
                      <code class="bg-red-100 text-red-700 px-1 rounded">5</code> — FAILED · Ошибка
                    </div>
                  </div>
                </div>
              <% end %>
